...
void Player::update(int deltaTime, const Enemy& enemy)
{
	...
	// Si el temporizador está activo, no permitir cambios de animación
	if (plorantTimer == 0) {
		// Código existente para manejar movimientos y animaciones...
		...
		if (checkCollision(enemy.getPosition(), glm::ivec2(16, 16)))
		{
			...
			if (isRightFacing()) {
				if (sprite->animation() != PLORANT_DRETA) {
					cout << "colision amb el plorant dreta" << endl;
					sprite->changeAnimation(PLORANT_DRETA);
					plorantTimer = 500; // 1 segundo en milisegundos
				}
			}
			else if (isLeftFacing()) {
				if (sprite->animation() != PLORANT_ESQUERRA) {
					cout << "colision amb el plorant esquerra" << endl;
